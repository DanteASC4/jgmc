...
export function linechart({
  ...
  const sum = dataLabels === "percentage"
    ? asNumerical.reduce((a, b) => a + b, 0)
    : 0;
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (lineTypeForLine === "straight") {
      ...
      for (let i = 0; i < coords.length - 1; i++) {
        ...
        } else {drawAttr += ` ${controls[0]},${controls[1]} ${controls[2]},${
            controls[3]
          } ${toC[0]},${toC[1]} `;}
      }
  ...
  if (isGradient && gradientColors && gradientMode && gradientId) {
    if (gradientMode === "individual") {
      ...
      const [maskId, theMask] = createLineChartMask(createdMaskingLines);
      const [gradDef, gradBg] = createLinearGradient(
        gradientColors,
        gradientDirection,
        gradientMode,
